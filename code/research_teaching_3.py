...
def summarize(df, category_col):
    ...
    for cat in ['RT Causality', 'TR Causality']:
    ...
    subset_bi = df[df[category_col] == 'Bidirectional']
    ...
    if category_col == 'category_combined':
        ...
        lag = int(category_col[-1])
        f_bi_rt = [v for v in subset_bi[f'rt{lag}_F'] if isinstance(v, float)]
        f_bi_tr = [v for v in subset_bi[f'tr{lag}_F'] if isinstance(v, float)]

    if f_bi_rt:
        summary.append(['Bidirectional-RT', count_bi, f"{perc_bi}%", round(np.mean(f_bi_rt), 2), round(np.std(f_bi_rt), 2),
                        round(np.median(f_bi_rt), 2), round(np.min(f_bi_rt), 2), round(np.max(f_bi_rt), 2)])
    if f_bi_tr:
        summary.append(['Bidirectional-TR', count_bi, f"{perc_bi}%", round(np.mean(f_bi_tr), 2), round(np.std(f_bi_tr), 2),
                        round(np.median(f_bi_tr), 2), round(np.min(f_bi_tr), 2), round(np.max(f_bi_tr), 2)])

    # No causality
    subset_no = df[df[category_col] == 'No Causality']
    count_no = len(subset_no)
    perc_no = round(count_no / total * 100, 1)
    f_values = []
    if category_col == 'category_combined':
        for lag in range(1, 3):
            f_values += [v for v in subset_no[f'rt{lag}_F'].tolist() + subset_no[f'tr{lag}_F'].tolist() if isinstance(v, float)]
    else:
        lag = int(category_col[-1])
        f_values += [v for v in subset_no[f'rt{lag}_F'].tolist() + subset_no[f'tr{lag}_F'].tolist() if isinstance(v, float)]
    avg_f = round(np.mean(f_values), 2) if f_values else '-'
    ...
    min_f = round(np.min(f_values), 2) if f_values else '-'
    max_f = round(np.max(f_values), 2) if f_values else '-'
    summary.append(['No Causality', count_no, f"{perc_no}%", avg_f, std_f, med_f, min_f, max_f])
...
headers = ['Category', 'Count', 'Percentage', 'Avg F', 'Std Dev', 'Median', 'Min', 'Max']
